...
export const disableSiteSsh = async (server, site, options = {}) => {
  ...
  const command = [
    ...
    '    [ -d "$parent" ] || break',
    '    setfacl -x "u:${USER_NAME}" "$parent" 2>/dev/null || true',
...
const buildProvisionCommand = ({ username, sitePath, publicKey }) => {
  ...
  return [
    ...
    '  while [ -n "$parent" ] && [ "$parent" != "/" ] && [ "$parent" != "." ]; do',
    '    [ -d "$parent" ] || break',
    '    setfacl -m "u:${USER_NAME}:--x" "$parent"',
    '    parent="$(dirname "$parent")"',
    '  done',
    '}',
...
const buildSiteTestCommand = ({ sitePath, sshUsername, marker }) => [
  ...
  'cleanup_temp() { rm -f "$TEST_OUTPUT" "$TARGETS_FILE" "$TARGET_FILE" "$ROOT_TARGET_FILE" 2>/dev/null || true; }',
  ...
  '',
  'ROOT_TARGET_FILE="$(mktemp /tmp/bt-ssh-site-root.XXXXXX)" || exit 1',
  'printf "%s" "$SITE_PATH" > "$ROOT_TARGET_FILE"',
  'chown "$SSH_USER" "$ROOT_TARGET_FILE" 2>/dev/null || true',
  'chmod 600 "$ROOT_TARGET_FILE" 2>/dev/null || true',
  'root_target="target=\\"\\$(cat \\"$ROOT_TARGET_FILE\\")\\""',
  'if run_as_site_user "$root_target; cd \\"\\$target\\""; then',
  '  emit pass "站点路径权限" "$SSH_USER 可进入 $SITE_PATH"',
  'else',
  '  emit fail "站点路径权限" "$(last_error)"',
  '  exit 0',
  'fi',
  'rm -f "$ROOT_TARGET_FILE" 2>/dev/null || true',
  'ROOT_TARGET_FILE=""',
  '',
  'printf "%s\\n" "$SITE_PATH" > "$TARGETS_FILE"',
  'find "$SITE_PATH" -mindepth 1 -maxdepth 4 -type d \\',
  '  ! -name ".git" ! -name "node_modules" ! -name "vendor" ! -name "cache" \\',
  '  ! -path "*/.git/*" ! -path "*/node_modules/*" ! -path "*/vendor/*" ! -path "*/cache/*" \\',
  '  2>/dev/null | head -n 8 >> "$TARGETS_FILE"',
  '',
